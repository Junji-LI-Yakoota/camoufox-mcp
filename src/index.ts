...
import { closeActiveSessions, handleSessionAction, handleSessionClose, handleSessionNavigate, handleSessionResume, handleSessionSaveHtml, handleSessionScreenshot, handleSessionSnapshot, handleSessionStart } from "./sessions.js";
...
registerJsonTool("browse_session_screenshot", "Capture a bounded screenshot of an existing browser session without navigating.", sessionScreenshotToolShape, readOnlyOpenWorld, async (input) => handleSessionScreenshot(input as SessionScreenshotToolInput));
registerJsonTool("browse_session_save_html", "Save the current page of an existing browser session as a standalone human-readable HTML file on disk, with same-origin stylesheets inlined.", sessionSaveHtmlToolShape, readOnlyOpenWorld, async (input) => handleSessionSaveHtml(input as SessionSaveHtmlToolInput));
